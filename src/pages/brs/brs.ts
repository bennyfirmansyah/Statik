...
export class BrsPage {  
	...
	constructor(public navCtrl: NavController, public loadingController:LoadingController, 
		public modalCtrl:ModalController, public publicationList: BRS) {
	}
	...
	getOuterName(event){
		...
		if(this.getSelectedValue!=""){   //Jika filter tahun tidak kosong
			if(this.keyword!=""){ //Jika keyword tidak kosong
				this.publicationList.filterSearchBRS(this.keyword,this.keyTahun,this.page).subscribe((publicationListData:any) => {
					this.allPublication= publicationListData.data;
				...
				this.publicationList.filterBRS(this.keyTahun,this.page).subscribe((publicationListData:any) => {
					this.allPublication= publicationListData.data;
					if(this.allPublication.length>0){		
						allIndicatorLoadingController.dismiss();	
						this.pages=this.allPublication[0].pages;
			...
			if(this.keyword!=""){ //Jika keyword tidak kosong
				this.publicationList.searchBRS(this.keyword,this.page).subscribe((publicationListData:any) => {
					this.allPublication= publicationListData.data;
					if(this.allPublication.length>0){
						allIndicatorLoadingController.dismiss();
						this.pages=this.allPublication[0].pages;
	...
	launchPubDetailsPage(pub){
		...
		let pubModal = this.modalCtrl.create('BrsdetilPage', pub);
		pubModal.present();
	}
